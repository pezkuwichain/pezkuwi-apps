...
IPFS allows sharing files in a decentralized manner in a similar fashion the pezkuwi network exchanges blocks. IPFS works best when many nodes seed the same data. Nodes can seed specific data by **pinning** them.
...
```
$ /usr/local/bin/pezkuwijs-ipfs-pin.sh
...
SHELL=/bin/bash
HOME=/
0 * * * * /usr/local/bin/pezkuwijs-ipfs-pin.sh >/dev/null 2>&1
```
...
Qmd8UnRQiBobm4qb6dhiC1HoQ7SvwZrWJenoN3JPEV3iiF 480594  pezkuwijs.3af757ad.js
...
* Mac: `~/Library/Application Support/pezkuwi-apps/pezkuwi-accounts`
